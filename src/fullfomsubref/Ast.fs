...
let rec printTyType outer ctx tyT =
  ...
  | TySource tyT -> (pr "Source "; printTyAType false ctx tyT)
  | TySink tyT -> (pr "Sink "; printTyAType false ctx tyT)
  | TyAbs (tyX, knK1, tyT2) ->
...
and printtmAppTerm outer ctx t =
  ...
  | TmApp (_, t1, t2) ->
      (obox0 ();
       ...
       cbox ())
  | TmRef (_, t1) ->
      (obox (); pr "ref "; printTerm false ctx t1; cbox ())
  | TmDeref (_, t1) ->
      (obox (); pr "!"; printTerm false ctx t1; cbox ())
  | TmTimesfloat (_, _, t2) ->
      (pr "timesfloat ";
       printTerm false ctx t2;
       pr " ";
